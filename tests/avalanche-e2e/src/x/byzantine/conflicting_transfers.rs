...
pub const NAME: &str = "X_BYZANTINE_CONFLICTING_TRANSFERS";
...
pub async fn run(spec: Arc<RwLock<Spec>>) -> io::Result<()> {
    ...
    for base_tx in base_txs.iter() {
        ...
        let resp = avalanche_sdk_x::issue_tx(&rpc_eps[0], &hex_tx)
            .await
            .unwrap();

        let tx_id = resp.result.unwrap().tx_id;
        log::info!("issued {}", tx_id);
    ...
    // only one TX can be accepted, and rest should timeout
    // liveness is not guaranteed for rogue transactions
    // the avalanche engine validates such transaction
    // so they are filtered out even before issue
    if accepted.len() != 1 {
        return Err(Error::new(
            ErrorKind::Other,
            format!("found {} accepted, expected 1", accepted.len()),
        ));
    }
